...
After the Compact contract compiles successfully:
```bash
cd /mnt/c/Users/John/utils_Midnight/midnight-kitties_folder
yarn build
```

## Step 7: Deploy the NFT Contract

Run the interactive CLI with the remote proof server (testnet):
```bash
yarn kitties-cli-remote-ps
```

Use the CLI to deploy your NFT contract. **Save the contract address** - John needs it for the quiz!

## What John Needs

1. Screenshot of successful compile
2. **Contract address** from the deployment (this is the quiz answer!)
...
*Updated by Cassie on Chuck - Dec 16, 2025*
